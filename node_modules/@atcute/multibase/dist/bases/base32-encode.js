...
export const toBase32 = (bytes) => {
    ...
    for (let g = 0; g < pairs; g++) {
        ...
        str += _fromCharCode(cc[a0 >>> 3], cc[((a0 << 2) | (a1 >>> 6)) & 0x1f], cc[(a1 >>> 1) & 0x1f], cc[((a1 << 4) | (a2 >>> 4)) & 0x1f], cc[((a2 << 1) | (a3 >>> 7)) & 0x1f], cc[(a3 >>> 2) & 0x1f], cc[((a3 << 3) | (a4 >>> 5)) & 0x1f], cc[a4 & 0x1f], cc[b0 >>> 3], cc[((b0 << 2) | (b1 >>> 6)) & 0x1f], cc[(b1 >>> 1) & 0x1f], cc[((b1 << 4) | (b2 >>> 4)) & 0x1f], cc[((b2 << 1) | (b3 >>> 7)) & 0x1f], cc[(b3 >>> 2) & 0x1f], cc[((b3 << 3) | (b4 >>> 5)) & 0x1f], cc[b4 & 0x1f]);
        ip += 10;
    }
    // remaining full group if odd count
    if (full & 1) {
        const b0 = bytes[ip], b1 = bytes[ip + 1], b2 = bytes[ip + 2], b3 = bytes[ip + 3], b4 = bytes[ip + 4];
        str += _fromCharCode(cc[b0 >>> 3], cc[((b0 << 2) | (b1 >>> 6)) & 0x1f], cc[(b1 >>> 1) & 0x1f], cc[((b1 << 4) | (b2 >>> 4)) & 0x1f], cc[((b2 << 1) | (b3 >>> 7)) & 0x1f], cc[(b3 >>> 2) & 0x1f], cc[((b3 << 3) | (b4 >>> 5)) & 0x1f], cc[b4 & 0x1f]);
        ip += 5;
    }
